...
0. **Ensure dependencies.** On the first invocation in a session, invoke `ensure-deps`. If the venv is missing, run the check with system `python3` (the check script is designed to run that way) — don't improvise. If the verdict is `not_ready`, **offer to fix it in one step, and on a yes, run the setup yourself**: create the venv, then `./.venv/bin/python3 -m pip install -r .claude/skills/ensure-deps/requirements.txt`. That requirements file already exists and is the source of truth — never hand-pick package names, and never claim it's missing. Re-run the check, and only proceed once it passes. Don't present a multi-option menu or ask the user to paste commands themselves unless they decline the offer. Skip this step on subsequent runs in the same session.
