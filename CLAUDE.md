...
component package from GitHub Packages. **Never edit the root `index.html` or `assets/`
directly**: those are the built output that GitHub Pages serves, regenerated by `make build`.
Edit the React source instead:
...
Then `make build`. Installing the package needs a GitHub token with `read:packages`:
`export GITHUB_TOKEN=$(gh auth token)` (see `.npmrc`). The old htm template + Python asset
scripts under `.claude/skills/onboarding-guide/` are retained for reference but are no longer
the build path.
